...
# 训练和评估函数
def train_evaluate(hidden_size, num_layers, num_attention_heads, lr):
    ...
    model = TransformerRegressor(hidden_size=hidden_size, num_layers=num_layers, num_attention_heads=num_attention_heads).to(device)
...
# 初始化贝叶斯优化
optimizer = BayesianOptimization(f=train_evaluate, pbounds=pbounds, random_state=1)
...
with open('./submit_trans.txt', 'w') as fw:
    fw.writelines('\n'.join(ans_str_lst))
