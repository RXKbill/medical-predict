...
# 使用LightGBM的Regressor和GridSearchCV
lgbm = lgb.LGBMRegressor(boosting_type='gbdt', objective='regression', random_state=42, device='gpu')
grid_search_lgb = GridSearchCV(lgbm, param_grid_lgb, cv=5, verbose=2, n_jobs=-1)
# 执行网格搜索
...
with open('./submit_gbm.txt','w') as fw:
    fw.writelines('\n'.join(ans_str_lst))